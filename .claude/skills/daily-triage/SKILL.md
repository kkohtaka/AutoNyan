...
post it again — **but it still goes into the digest under `あなたの判断待ち`, every
run** (Step 8). Take the date it was first raised from the `created_at` of the
earliest footer-carrying comment that asks it, so the digest can show how long it
has been waiting.
...
And per decision: every one waiting on the maintainer, raised this run or on any
earlier one, is in the digest under `あなたの判断待ち`. That list is deliberately
not de-duplicated across runs — an entry leaves it only when the decision is
answered, never because it has been reported before.
...
**Send nothing when the run produced no new item and nothing is waiting on the
maintainer.** This is the rule that keeps the digest readable: silence means
"nothing changed, and nothing needs you". A run whose only content is
non-decision items already reported on earlier days does not send.

**A pending decision is the exception, and is reported every run** however many
times it has already been reported. A routine that cannot decide for itself has
exactly one thing it must keep visible, and an unanswered decision is precisely
the item that would otherwise sit indefinitely with nobody reminded of it. So
whenever heading 2 has content, the digest sends, even when nothing else changed
that run.

**This changes Slack only.** Step 4's re-send suppression on GitHub is
untouched: a question already asked is still never re-posted as a new issue
comment. The daily repetition belongs in the digest, which is a notification —
not in the system of record, where it would be noise.

Include only the headings that have content:

1. **本日の成果** — issues and pull requests created or advanced: number, title,
   link, state.
2. **あなたの判断待ち** — every decision currently waiting on the maintainer,
   whether raised this run or on an earlier one: one line each with what needs
   deciding, a recommendation, and the date it was first raised. **This heading
   is not filtered by "already reported"** — that is what makes it daily.
...
4. **継続中** — *non-decision* items already reported earlier: work blocked on an
   environment or a permission, and anything else carried forward. One line each
   with the date first raised, never re-expanded. A decision belongs under
   heading 2, not here.
