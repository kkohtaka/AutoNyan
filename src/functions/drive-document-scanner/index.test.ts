...
import { driveDocumentScanner } from './index';
...
describe('driveDocumentScanner', () => {
  ...
  beforeEach(() => {
    ...
    (mockGoogle.auth.GoogleAuth as jest.MockedClass<typeof google.auth.GoogleAuth>).mockImplementation(() => ({} as any));
  ...
  describe('HTTP request', () => {
    ...
    it('should return 400 when folderId is missing', async () => {
      ...
      await driveDocumentScanner(mockRequest as Request, mockResponse as Response);
    ...
    it('should successfully scan folder and publish documents', async () => {
      ...
      expect(responseBody.message).toContain('Successfully scanned folder test-folder-id and found 2 document files');
      ...
      expect(mockDriveList).toHaveBeenCalledWith({
        q: expect.stringContaining("'test-folder-id' in parents and trashed=false"),
    ...
    it('should accept parameters from query string', async () => {
      ...
      expect(responseStatus).toBe(200);
      expect(responseBody.message).toContain('Successfully scanned folder query-folder-id and found 0 document files');
    });
  });
  ...
  describe('CloudEvent request', () => {
    it('should throw error when folderId is missing', async () => {
      ...
      await expect(driveDocumentScanner(cloudEvent)).rejects.toThrow(
    ...
    it('should successfully process CloudEvent and return result', async () => {
      ...
      const result = await driveDocumentScanner(cloudEvent);

      expect(result).toBeDefined();
      expect(result!.message).toContain('Successfully scanned folder test-folder-id and found 1 document files');
  ...
  describe('Document type filtering', () => {
    it('should include all supported document MIME types in search query', async () => {
      ...
      await driveDocumentScanner(mockRequest, mockResponse);
      ...
      expect(query).toContain("mimeType='application/vnd.openxmlformats-officedocument.wordprocessingml.document'");
